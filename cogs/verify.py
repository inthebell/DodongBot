...
def check_answer(
    ...
    if question["type"] == "nickname":
        ...
        return (
            ...
            == normalize_answer(server_nickname)
        )
...
class VerifyAnswerModal(discord.ui.Modal):
    def __init__(self, user_id: int):
        ...
        super().__init__(
            title=f"입주 인증 {self.question_index + 1}/10"
        )
    ...
    async def on_submit(
        ...
        if (
            ...
            if re.match(pattern, answer) is None:
                ...
                await interaction.response.send_message(
                    "❌ 닉네임 확인에 실패했습니다.\n\n"
                    "입력한 닉네임과 현재 도동마을 서버 별명이 "
                    "일치하지 않습니다.\n\n"
                    "먼저 **도동마을 디스코드 서버 별명을 "
                    "인게임 닉네임과 동일하게 변경**한 후 "
                    "다시 인증해주세요.\n\n"
